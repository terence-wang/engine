...
var Viewer = function (canvas) {
    ...
    app.on('update', function () {
        ...
        else
        {
            ...
            {
                autoLoad = false;
                app.assets.loadFromUrl("../../examples/assets/models/skirt_test/skirt_test.json", "model", function (err, asset) {
                    self._onLoaded(err, asset);
                    app.assets.loadFromUrl("../../examples/assets/models/skirt_test/Bottom_Skirt_Pleated.glb", "container", function (err, asset) {
                        self._onLoaded(err, asset);
                        app.assets.loadFromUrl("../../examples/assets/models/skirt_test/RunwayWalk.glb", "container", function (err, asset) {
                            //self._onLoaded(err, asset);
                        });
                    });
                });
    ...
    _onLoaded: function (err, asset) {
        if (!err) {
            ...
            if (!this.entity)
            {
                if (asset.type === 'model')
                {
                    var ref_entity = new pc.Entity();
                    ref_entity.addComponent("model", {
                        type: "asset",
                        asset: asset,
                        castShadows: true
                    });

                    this.ref_entity=ref_entity;
                }
                else
                {
                ...
                if (entity && entity.model && entity.model.meshInstances && entity.model.meshInstances.length>0)//resource.model && resource.model.meshInstances.length>0)
                {
                    ...
                    this.sv = this.createSoftVolume(this.vm, 1.5, 1.0, 0.05);
